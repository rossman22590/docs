...
**Description:**
When enabled, using the same settings will produce the same image every time. This is helpful if you’re refining a particular prompt and want consistent results to compare adjustments. When disabled, the image may vary with each generation, introducing an element of surprise and variety.
